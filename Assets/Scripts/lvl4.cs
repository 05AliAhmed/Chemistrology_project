...
public class lvl4 : MonoBehaviour
{
    ...
    public void ScoreSystem() // will late be connected with shooting system to count scores | Score Button
    {
        ...
        if(score > PlayerPrefs.GetInt("Highscore4",0)) // if score is greater than H.S, 0 is there because at start high score is 0.
        {
            PlayerPrefs.SetInt("Highscore4", score); // setting Highscore to be the score continously 
    ...
    void Start()
    {
        ...
        highScoreTxt.text = PlayerPrefs.GetInt("Highscore4", 0).ToString();
        Time.timeScale = 1f;
